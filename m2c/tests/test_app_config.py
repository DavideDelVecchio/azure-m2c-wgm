...
# Modify this when testing from another directory location
M2C_REPO_DIR = os.environ['M2C_REPO_DIR']

def absolute_filename(path):
    return '{}/{}'.format(M2C_REPO_DIR, path)
...
def test_instance_variables():
    ...
    assert(app_config.artifacts_dir == absolute_filename('reference_app/artifacts'))
    assert(app_config.data_dir == absolute_filename('reference_app/data'))
...
def test_metadata_file():
    app_config = AppConfig()
    expected = absolute_filename('reference_app/data/metadata/olympics_metadata.json')
...
def test_metadata_files():
    app_config = AppConfig()
    expected = sorted([
        absolute_filename('reference_app/data/metadata/openflights_metadata.json'),
        absolute_filename('reference_app/data/metadata/olympics_metadata.json')
    ])
...
def test_mapping_file():
    app_config = AppConfig()
    expected =  absolute_filename('reference_app/data/metadata/olympics_mapping.json')
...
def test_mongoexports_dir():
    app_config = AppConfig()
    expected = absolute_filename('reference_app/data/olympics/mongoexports')
...
def test_mongoexports_file():
    app_config = AppConfig()
    expected = absolute_filename('reference_app/data/olympics/mongoexports/olympics__countries__source.json')
...
def test_artifact_dir():
    app_config = AppConfig()
    expected = absolute_filename('reference_app/artifacts/shell')
